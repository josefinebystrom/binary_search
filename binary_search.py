...
def count_repeats(xs, x):
    ...
    if min_less_than_x(xs, x) is None:
        less = 0
    else:
        less = min_less_than_x(xs, x) + 1
    if min_bigger_than_x(xs, x) is None:
        greater = 0
    else:
        greater = min_bigger_than_x(xs, x)
    if greater == 0 and less == 0:
        return len(xs)
    return abs(less - greater)
